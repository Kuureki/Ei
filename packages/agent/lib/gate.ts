// config-backed gates shared by the healing and lineage schedules.
...
export async function readGate(ex: SqlExecutor, key: string): Promise<unknown> {
  const r = await ex.query(`select value from config where key = $1`, [key]);
...
export async function writeGate(ex: SqlExecutor, key: string, value: unknown): Promise<void> {
  await ex.query(
    `insert into config (key, value, version) values ($1, $2::jsonb, 1)
